...
from models.CoNeMOS.unet3d import UNet3D
from models.CoNeMOS import loss
...
def parse_args():
    desc = "Pytorch implementation of CoNeMOS"
    ...
    parser.add_argument('--workspace', type=str, default='./exp/CoNeMOS/checkpoint')
    # GPU config
    ...
    for epoch in range(args.start_epoch, args.epoches):
        ...
        for batch_idx, sample in tqdm.tqdm(
            ...
            for i in range(label.shape[0]):
                target_class = torch.unique(label[i])
                target_class = target_class[target_class != 0]
                if target_class.shape[0] == 0:
                    continue
            
                rand_cls = target_class[random.randint(0, target_class.shape[0]-1)]
