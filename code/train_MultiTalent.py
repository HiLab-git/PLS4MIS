...
from models.MultiTalent.multitalent import SampleClassAdaptiveLoss
...
def parse_args():
    desc = "Pytorch implementation of MultiTalent"
    parser = argparse.ArgumentParser(description=desc)
    # dir config
    parser.add_argument('--exp_dir', type=str, default='./exp/WORD/MultiTalent')
    parser.add_argument('--data_dir', type=str, default='./datasets/WORD')
    parser.add_argument('--workspace', type=str, default='./exp/WORD/MultiTalent/checkpoint')
    # GPU config
...
def train(model, train_loader, val_loader, writer, args):
    ...
    mt_loss = SampleClassAdaptiveLoss()
    ...
    for epoch in range(args.start_epoch, args.epoches):
        ...
        for batch_idx, sample in tqdm.tqdm(
            ...
            # Compute multitalent loss
            seg_loss = mt_loss(seg_pred, label)
