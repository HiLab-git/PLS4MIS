...
def parse_args():
    desc = "Pytorch implementation of UNet"
    parser = argparse.ArgumentParser(description=desc)
    # dir config
    parser.add_argument('--exp_dir', type=str, default='./exp/WORD/UNet')
    parser.add_argument('--data_dir', type=str, default='./datasets/WORD')
    parser.add_argument('--workspace', type=str, default='./exp/WORD/UNet/checkpoint')
    # GPU config
...
def train(model, train_loader, val_loader, writer, args):
    ...
    for epoch in range(args.start_epoch, args.epoches):
        ...
        for batch_idx, sample in tqdm.tqdm(
            ...
            seg_pred_soft = torch.clamp(seg_pred_soft, min=1e-10, max=1)

            optimizer.zero_grad()
            
            # Compute dice
            dice_loss = dice_criterion(seg_pred_soft.permute(0, 2, 3, 4, 1), label.permute(0, 2, 3, 4, 1),
                                        num_class=args.num_classes)
            ce_loss = ce_criterion(seg_pred, label)
            seg_loss = 0.5 * (dice_loss + ce_loss)
        ...
        logging.info(
            '\n Epoch[%4d/%4d]-Lr: %.6f -->for ct_array in ct_array_list: Train...' % (epoch + 1, args.epoches, 
        ...
        if (epoch + 1) % args.val_interval == 0:
            ...
            if torch.mean(val_dice.avg) >= torch.mean(best_dice):
                ...
                torch.save(model.state_dict(), best_model_path) # parallel: model.module.state_dict()
                logging.info(
    ...
    torch.save({'model': model.state_dict(), 'optim': optimizer.state_dict()}, final_model_path) # parallel: model.module.state_dict()
