...
class UNet3D_URPC(nn.Module):
    ...
    * Reference: Özgün Çiçek, Ahmed Abdulkadir, Soeren S. Lienkamp, Thomas Brox, Olaf Ronneberger:
    ...
    def __init__(self, params):
        super(UNet3D_URPC, self).__init__()
        self.params    = params
        ...
        # deep supervision
        self.dsv4 = UnetDsv3(
            in_size=self.ft_chns[3], out_size=self.n_class, scale_factor=8)
        self.dsv3 = UnetDsv3(
            in_size=self.ft_chns[2], out_size=self.n_class, scale_factor=4)
        self.dsv2 = UnetDsv3(
            in_size=self.ft_chns[1], out_size=self.n_class, scale_factor=2)
        self.dsv1 = nn.Conv3d(
...
class UNet3D_URPC_LearnPA(nn.Module):
    """
    An implementation of the U-Net.
    ...
    * Reference: Özgün Çiçek, Ahmed Abdulkadir, Soeren S. Lienkamp, Thomas Brox, Olaf Ronneberger:
    ...
    """
    def __init__(self, params):
        super(UNet3D_URPC_LearnPA, self).__init__()
        self.params    = params
        self.in_chns   = self.params['in_chns']
        self.ft_chns   = self.params['feature_chns']
        self.dropout   = self.params['dropout']
        ...
        self.param = nn.Parameter(torch.Tensor([2, 2, 0.5, 0.5]))
