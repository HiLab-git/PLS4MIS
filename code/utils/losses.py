...
def unlabel_class_entropy_loss(p, mask, C=2):
    ## p N*C*W*H*D
    # Compute unlabeled class entropy
    y1 = -1 * torch.sum(p * torch.log(p + 1e-6), dim=1) / torch.tensor(np.log(C)).cuda()
    y1_m = y1 * mask
...
def Orthogonal_loss(f_di_pool, f_ds_pool):
    loss_orthogonal = (f_di_pool.square() * f_ds_pool.square()).mean()
    return loss_orthogonal

def l1_mse_loss(y_pred, y_true):
    mse = torch.mean((y_pred - y_true) ** 2)
    l1 = torch.mean(torch.abs(y_pred - y_true))
    return (mse + l1) / 2

def CO_Contrastive(outputsSoft):
    
    # learn class relationship information, sr means "softmax + reshape"
    b, c, d, h, w = outputsSoft.shape
    outputsSoft_sr = outputsSoft.reshape(b, c, -1)
    outputsSoft_sr = F.normalize(outputsSoft_sr, p=2, dim=2)
    
    outputsSoft_sr_T = outputsSoft_sr.permute(0, 2, 1)
    affinity_ori = torch.bmm(outputsSoft_sr, outputsSoft_sr_T)

    # use CE Loss
    diag = torch.eye(c, dtype=torch.float32, device=affinity_ori.device).unsqueeze(0).expand(b, -1, -1)
    ce_criterion = torch.nn.CrossEntropyLoss()
    loss_ce = ce_criterion(affinity_ori, diag)
...
class DistillationLoss(nn.Module):
    ...
    def forward(self, y_pred, y_gt):
        b, _, d, h, w = y_pred.shape
        
        y_pred_t = y_pred.div(self.T)
        y_gt_t = y_gt.div(self.T)

        soft_loss = torch.sum(-(self.softmax(y_gt_t) * self.logsoftmax(y_pred_t))) / (b*d*h*w)
