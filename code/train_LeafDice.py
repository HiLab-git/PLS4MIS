...
from models.LeafDice.leaf_dice_loss import LeafDiceLoss
...
def parse_args():
    desc = "Pytorch implementation of LeafDice"
    parser = argparse.ArgumentParser(description=desc)
    # dir config
    parser.add_argument('--exp_dir', type=str, default='./exp/LeafDice')
    parser.add_argument('--data_dir', type=str, default='./dataset/word3d_ld')
    parser.add_argument('--workspace', type=str, default='./exp/LeafDice/checkpoint')
    # GPU config
    ...
    parser.add_argument('--patch_size', type=list, default=[128, 128, 96]) #[x, y, z]
...
def train(model, train_loader, val_loader, writer, args):
    ...
    for epoch in range(args.start_epoch, args.epoches):
        ...
        for batch_idx, sample in tqdm.tqdm(
            ...
            # Compute loss
            seg_loss = criterion(seg_pred, label)
